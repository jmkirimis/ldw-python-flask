...
from models.database import db, Character
import urllib
import json
import locale
...
def init_app(app):
    ...
    @app.route('/cadcharacter/delete/<int:id>', methods=['GET', 'POST'])
    def cadcharacters(id=None):
        if id:
        ...
        if request.method == 'POST':
            newcharacter = Character(request.form['nome'], request.form['ki'], request.form['kiMax'], request.form['raca'])
            ...
            per_page = 5
            characters_page = Character.query.paginate(page=page, per_page=per_page)
            return render_template('cadcharacters.html', characters_page=characters_page)
    ...
    def edit(id):
        ...
        if request.method == 'POST':
            character.nome = request.form['nome']
            character.ki = request.form['ki']
            character.kiMax = request.form['kiMax']
            character.raca = request.form['raca'] 
            db.session.commit()
            return redirect(url_for('cadcharacters'))
